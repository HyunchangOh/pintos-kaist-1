...
setup_stack (struct intr_frame *if_) {
	bool success = false;
	void *saved_sp = (void *) (((uint8_t *) USER_STACK) - PGSIZE);
    ...
    if (vm_alloc_page(VM_ANON | VM_MARKER_0, saved_sp, 1))
    {

		success = vm_claim_page(saved_sp);
		
		if (success){
			// printf("here??\n");
			if_->rsp = USER_STACK;
            thread_current()->saved_sp = saved_sp;
		}
